...
impl SshManager {
    ...
    fn load_connections(path: &PathBuf) -> Result<HashMap<String, SshConnection>> {
        ...
        let contents = fs::read_to_string(path)
            .context("Failed to read SSH connections file")?;
        
        let connections: HashMap<String, SshConnection> = serde_json::from_str(&contents)
            .context("Failed to parse SSH connections")?;
    ...
    pub fn save_connections(&self) -> Result<()> {
        ...
        fs::write(&self.config_path, contents)
            .context("Failed to write SSH connections file")?;
